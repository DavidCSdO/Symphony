import { useRef, useState, useCallback, useEffect, PointerEvent as ReactPointerEvent, KeyboardEvent as ReactKeyboardEvent } from 'react';
import './OptionWheel.css';

export interface OptionWheelProps {
  items?: React.ReactNode[];
  defaultSelected?: number;
  selected?: number; // Added for controlled component sync
  onChange?: (index: number, item: any) => void;
  onConfirm?: (index: number, item: any) => void;
  textColor?: string;
  activeColor?: string;
  side?: 'left' | 'right';
  fontSize?: number;
...
export const OptionWheel = ({
  ...
  // Use `selected` if controlled, otherwise `defaultSelected`
  const initialSelected = selected !== undefined ? selected : defaultSelected;
  ...
  const handleItemClick = useCallback(
    (index: number) => {
      ...
      if (index === curIdx) {
        // Double-click/confirm interaction: user clicked the already focused item
        cfg.onConfirm?.(index, cfg.items[index]);
        return;
      }
  ...
  useEffect(() => {
    if (selected !== undefined && selected !== selectedRef.current && selected !== Math.round(targetRef.current)) {
